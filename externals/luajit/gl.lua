...
function gl.Clear(...)
	if select('#', ...) > 0 then
		...
		lib.glClear(bit.bor(lib.GL_COLOR_BUFFER_BIT, lib.GL_DEPTH_BUFFER_BIT))
	end
end
...
function gl.TexParameter(target, pname)
	if pname == lib.GL_TEXTURE_MAG_FILTER
		...
		or pname == lib.GL_TEXTURE_WRAP_S
		or pname == lib.GL_TEXTURE_WRAP_T
		or pname == lib.GL_TEXTURE_WRAP_R
		or pname == lib.GL_TEXTURE_PRIORITY
		or pname == lib.GL_TEXTURE_RESIDENT
		or pname == lib.GL_TEXTURE_COMPARE_MODE
		or pname == lib.GL_TEXTURE_COMPARE_FUNC
		or pname == lib.GL_DEPTH_TEXTURE_MODE
		or pname == lib.GL_GENERATE_MIPMAP then
		local params = ffi.new("GLuint[?]", 1)
		lib.glGetTexParameteriv(target, pname, params)
		return params[0]
	elseif pname == lib.GL_TEXTURE_MIN_LOD
		or pname == lib.GL_TEXTURE_MAX_LOD
		or pname == lib.GL_TEXTURE_BASE_LEVEL
		or pname == lib.GL_TEXTURE_MAX_LEVEL then
		local params = ffi.new("GLfloat[?]", 1)
		lib.glGetTexParameterfv(target, pname, params)
		return params[0]
	elseif pname == lib.GL_TEXTURE_BORDER_COLOR then
...
function gl.CreateShader(kind, code)
	...
	if code then
		...
		if status[0] == gl.FALSE then
			...
			lib.glGetShaderiv(shader, lib.GL_INFO_LOG_LENGTH, infoLogLength)
			local strInfoLog = ffi.new("GLchar[?]", infoLogLength[0] + 1)
			lib.glGetShaderInfoLog(shader, infoLogLength[0], nil, strInfoLog)
			local ln = 1
...
function gl.CreateProgram(...)
	...
	if #args > 0 then
		for s in ipairs(args) do
			lib.glAttachShader(program, s)
		end
		lib.glLinkProgram(program)
		
		local status = ffi.new("GLint[1]")
		lib.glGetProgramiv(program, lib.GL_LINK_STATUS, status)
		if status[0] == gl.FALSE then
			local infoLogLength = ffi.new("GLint[1]")
			lib.glGetProgramiv(program, lib.GL_INFO_LOG_LENGTH, infoLogLength)
			local strInfoLog = ffi.new("GLchar[?]", infoLogLength[0] + 1)
			lib.glGetProgramInfoLog(program, infoLogLength[0], nil, strInfoLog)
			error("gl.LinkProgram " .. ffi.string(strInfoLog))
		end
		
		for s in ipairs(args) do
			lib.glDetachShader(program, s)
		end
	end
	return program
end
...
function sketch.enter_ortho(x, y, w, h)
	if w then
		...
		lib.glPushMatrix()
		lib.glLoadIdentity()
		lib.glOrtho(x, x+w, y, y+h, -100, 100)
...
function gl.assert(msg)
	...
	if err ~= lib.GL_NO_ERROR then
		local ok, str = pcall(lib.gluErrorString, err)
